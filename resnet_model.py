...
class ResNet(object):
    ...
    def train(self):
        ...
        for e in range(1, self.eps + 1):
            _tr_img, _tr_lab = input.shuffle_data(self.training_data[0], self.training_data[1])

            training_acc = 0.0
            training_loss = 0.0

            for itr in range(self.train_itr):
                _tr_img_batch, _tr_lab_batch = input.next_batch(_tr_img, _tr_lab, self.bs, itr)
